    ...
    for LR in ${LRs[@]}; do
        for ALPHA in ${ALPHAs[@]}; do
            ...
            echo "weightDecay = $WEIGHT_DEACAY," \
                ...
                "lr = $LR," \
                "alpha = $ALPHA"
            python src/main_incremental.py --approach seed --pretrained --skip-ft-pretraining --gmms $GMMS --max-experts $MAX_EXPERTS --use-multivariate --nepochs $NEPOCHS --gpu $GPU --tau $TAU --batch-size $BATCH_SIZE --num-workers $NUM_WORKERS --datasets $DATASET --num-tasks $NUM_TASKS --lr $LR --weight-decay $WEIGHT_DEACAY --alpha $ALPHA --use-test-as-val --network $NETWORK --momentum $MOMENTUM --exp-name $EXP_NAME --seed $SEED
        done
    done
done